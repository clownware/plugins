...
**Secret files on disk vs gitignore:** !`out=$(present=$(find . -maxdepth 1 -type f \( -name ".env" -o -name ".env.*" \) ! -name "*.example" ! -name "*.tpl" ! -name "*.sample" ! -name "*.template" 2>/dev/null); if [ -z "$present" ]; then echo "none present on disk (templates only or absent)"; else printf "%s\n" "$present" | while IFS= read -r f; do if git check-ignore -q "$f" 2>/dev/null; then echo "${f#./} gitignored"; else echo "${f#./} NOT gitignored — one commit from a leak"; fi; done; fi); echo "${out:-secret-file check produced no output}"`
**Secret delivery mechanism:** !`out=$(rg -l -i "op run|op://|op inject|sops|vault kv|doppler|aws secretsmanager|gcp secret|azure keyvault|1password" .env.tpl .env.example Taskfile.yml Makefile justfile docker-compose.yml .github/workflows 2>/dev/null | head -5); echo "${out:-no secrets-manager references — plaintext env is likely the only mechanism (hardening opportunity, not a vuln)}"`
...
- **Platform & infra** — security headers + CSP (`unsafe-inline`/`unsafe-eval`?); debug/metrics/pprof exposure and the constant-timeness of any guard token; error/log information leakage (internal errors or PII to clients/logs); config and secret handling (insecure defaults; secrets in history — grep contents, not just filenames; **gitignore coverage** — a secret file present on disk but not ignored is one `git add` from an incident; **delivery posture** — whether secrets are runtime-injected from a manager or materialized as plaintext on disk, see the severity rule below); Docker (non-root? secrets in layers?); CI/CD (mutable-tag vs SHA-pinned actions, `pull_request_target` secret exposure, `GITHUB_TOKEN` scope); server hardening (timeouts, body-size limits); DB (TLS/`sslmode`, pool, migration/RLS correctness).
...
- **Secret hygiene is graded, never binary.** A real secret committed to git or its history is **critical** (it is public, forked, and cached — flag rotation). A secret file present on disk but **not** gitignored is **medium** (imminent-leak risk; the fix is a `.gitignore` entry). A **gitignored** plaintext `.env` on a dev machine is the accepted **baseline** — recommend runtime injection (`op run`, sops, Vault, Doppler, cloud secret managers) as **informational** hardening, never as a vulnerability. Do not flag a normal gitignored `.env` as a finding; that is the over-severitization this audit exists to avoid.
