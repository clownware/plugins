...
3. **The mark's rules are law.** Do not add fill or outlines to the stroke master, recolor it (official accent variant excepted, digital only), or distort it. Use the supplied official filled artwork from `assets/logo/` unchanged for static placement; use the stroke master for draw-on animation. Clearspace = one stroke-square. Two lockups only: bare emblem, and emblem + wordmark horizontal.
